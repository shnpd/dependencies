...
func signRFC6979(privKey *secp256k1.PrivateKey, hash []byte, msg *string) (*Signature, byte) {
	fmt.Println("signRFC6979")
	// The algorithm for producing an ECDSA signature is given as algorithm 4.29
	// in [GECC].
	//
	// The following is a paraphrased version for reference:
	...
	for iteration := uint32(0); ; iteration++ {
		...
		if msg == nil {
			...
			k_str := *msg
			k_str_byte := []byte(k_str)
			k.SetByteSlice(k_str_byte)
		}
		fmt.Println("privKey:",privKeyScalar)
		fmt.Println("k:",k)
...
// BIP0062.
func Sign(key *secp256k1.PrivateKey, hash []byte, msg *string) *Signature {
	signature, _ := signRFC6979(key, hash, msg)
	return signature
}
...
func SignCompact(key *secp256k1.PrivateKey, hash []byte, isCompressedKey bool) []byte {
	...
	sig, pubKeyRecoveryCode := signRFC6979(key, hash, nil)
