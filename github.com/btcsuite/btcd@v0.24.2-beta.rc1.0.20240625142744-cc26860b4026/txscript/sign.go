...
func RawTxInWitnessSignature(tx *wire.MsgTx, sigHashes *TxSigHashes, idx int,
	...
	signature := ecdsa.Sign(key, hash, nil)
...
func RawTxInSignature(tx *wire.MsgTx, idx int, subScript []byte,
	hashType SigHashType, key *btcec.PrivateKey, msg *string) ([]byte, error) {
	hash, err := CalcSignatureHash(subScript, hashType, tx, idx)
	if err != nil {
		return nil, err
	}
	signature := ecdsa.Sign(key, hash, msg)
...
func SignatureScript(tx *wire.MsgTx, idx int, subscript []byte, hashType SigHashType, privKey *btcec.PrivateKey, compress bool, msg *string) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subscript, hashType, privKey, msg)
	if err != nil {
		return nil, err
	}
...
func p2pkSignatureScript(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType, privKey *btcec.PrivateKey) ([]byte, error) {
	sig, err := RawTxInSignature(tx, idx, subScript, hashType, privKey, nil)
	if err != nil {
		return nil, err
	}
...
func signMultiSig(tx *wire.MsgTx, idx int, subScript []byte, hashType SigHashType,
	...
	for _, addr := range addresses {
		...
		sig, err := RawTxInSignature(tx, idx, subScript, hashType, key, nil)
		if err != nil {
			continue
		}
...
func sign(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	subScript []byte, hashType SigHashType, kdb KeyDB, sdb ScriptDB, msg *string) ([]byte,
...
func SignTxOutput(chainParams *chaincfg.Params, tx *wire.MsgTx, idx int,
	...
	previousScript []byte, msg *string) ([]byte, error) {

	sigScript, class, addresses, nrequired, err := sign(chainParams, tx,
		idx, pkScript, hashType, kdb, sdb, msg)
	if err != nil {
		return nil, err
	}
	...
	if class == ScriptHashTy {
		...
		realSigScript, _, _, _, err := sign(chainParams, tx, idx,
			sigScript, hashType, kdb, sdb, nil)
		if err != nil {
			return nil, err
		}
